...
int connect_server(pid_t server_pid, int flags, int *recv, int *send) {
  ...
  if ((server_pid = connect_server_queue(server_pid, flags)) == -1)
    return -1;

  tmp = connect_process_comm(own_pid, server_pid, O_WRONLY);
  if (tmp == -1) {
  ...
  *send = tmp;
  ...
  tmp = connect_process_comm(server_pid, own_pid, O_RDONLY);
  if (tmp == -1) {
    kill(server_pid, SIGUSR2);
    return -1;
  }
